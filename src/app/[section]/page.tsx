...
export default async function Page({ params }: { params: Promise<{ section: string }> }) {
  ...
  const accountLabel = section === "agents" ? "Become an Agent" : section === "farmers" ? "Create Farmer account" : section === "markets" ? "Join the network" : "Create account";
  return <main className="sub"><nav className="subNav"><a className="brand" href="/">TRIMEX <small>INC.</small></a><div><a href="/platform">Platform</a><a href="/farmers">Farmers</a><a href="/agents">Agents</a><a href="/markets">Markets</a></div><a className="subAccount" href="/auth">Create account ↗</a></nav><header><p className="eyebrow">Trimex {section}</p><h1>{page[0]}</h1><p>Built as part of one connected Trimex platform—simple on the surface, with the right operational depth underneath.</p><a className="button dark" href="/auth">{accountLabel} ↗</a></header><section className="subContent"><p className="eyebrow">What this unlocks</p><div className="pills">{page[1].map((item,index)=><article key={item}><span>0{index+1}</span><h2>{item}</h2><p>Designed to make the agricultural network work more intelligently together.</p></article>)}</div></section><section className="subCta"><p className="eyebrow light">Trimex Inc.</p><h2>The future of African agriculture will be connected.</h2><a className="button lime" href="/auth">Create your account ↗</a></section></main>;
}
